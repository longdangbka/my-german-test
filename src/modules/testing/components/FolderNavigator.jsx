...
const FolderNavigator = ({ onTestSelect, searchTerm = '' }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Root level files */}
      {sortFiles(regularFiles).map(file => renderFile(file))}
      
      {/* Folders */}
      {Object.values(folderStructure.folders).map(folder => renderFolder(folder))}
      ...
      {regularFiles.length === 0 && Object.keys(folderStructure.folders).length === 0 && !bookmarkFile && (
        <div className="text-center p-8 text-gray-500 dark:text-gray-400">
          ...
          <p className="text-sm mt-2">Add some Markdown files to your vault directory</p>
        </div>
      )}
    </div>
  );
